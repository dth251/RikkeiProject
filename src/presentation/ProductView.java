...
public class ProductView {
    ...
    private static void updateProductView(Scanner sc) {
        try {
            ...
            System.out.printf("Giá sản phẩm hiện tại: %,.2f VND\n", currentProduct.getPrice());
    ...
    private static void deleteProductView(Scanner sc) {
        try {
            ...
            System.out.printf(GREEN + "Đã tìm thấy sản phẩm: %s (Hãng: %s, Giá: %,.2f VND, Tồn kho: %d)\n" + RESET,
    ...
    private static void printProductTable(List<Product> list) {
        ...
        System.out.printf("| %-5s | %-35s | %-15s | %-18s | %-10s |\n", "ID", "Tên sản phẩm", "Hãng", "Giá tiền (VND)", "Tồn kho");
        System.out.println(separator);

        for (Product p : list) {
            System.out.printf("| %-5d | %-35s | %-15s | %,18.2f | %-10d |\n",
